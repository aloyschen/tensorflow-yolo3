...
class yolo:
    ...
    def _conv2d_layer(self, inputs, filters_num, kernel_size, name, weights_dict = None, strides = 1):
        ...
        if weights_dict != None:
            ...
            use_bias = False, kernel_initializer = weight, name = name)
        else:
            conv = tf.layers.conv2d(
            inputs = inputs, filters = filters_num,
            kernel_size = kernel_size, strides = [strides, strides],
            padding = ('SAME' if strides == 1 else 'VALID'),
            use_bias = False, kernel_initializer = tf.variance_scaling_initializer(), name = name)
        return conv
    ...
    def _Residual_block(self, inputs, filters_num, blocks_num, conv_index, weights_dict = None, training = True, norm_decay = 0.997, norm_epsilon = 1e-5):
        ...
        for _ in range(blocks_num):
            shortcut = layer
            layer = self._conv2d_layer(layer, filters_num // 2, kernel_size = 3, strides = 1, name = "conv2d_" + str(conv_index), weights_dict = weights_dict)
            layer = self._batch_normalization_layer(layer, weights_dict, name = "batch_normalization_" + str(conv_index), training = training, norm_decay = norm_decay, norm_epsilon = norm_epsilon)
            conv_index += 1
    ...
    def yolo_inference(self, inputs, num_anchors, num_classes, training = True):
        ...
        _, conv2d_75, _ = self._yolo_block(route1, 256, num_anchors * (num_classes + 5), conv_index = conv_index, training = training, norm_decay = self.norm_decay, norm_epsilon = self.norm_epsilon)
    ...
    def yolo_loss(self, yolo_output, y_true, anchors, num_classes, ignore_thresh = .5):
        ...
        for index in range(3):
            ...
            raw_true_wh = tf.log(y_true[index][..., 2:4] / anchors[anchor_mask[index]] * input_shape[::-1])
            raw_true_wh = tf.where(tf.greater(y_true[index][..., 2:4] / anchors[anchor_mask[index]] * input_shape[::-1], 0), raw_true_wh, tf.zeros_like(raw_true_wh))
