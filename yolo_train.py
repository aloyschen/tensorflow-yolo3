...
def train():
    ...
    loss, accuracy = model.yolo_loss(output, bbox_true, model.anchors, config.num_classes, config.ignore_thresh)
    tf.summary.scalar('loss', loss)
    ...
    with tf.Session(config = tf.ConfigProto(log_device_placement = False)) as sess:
        ...
        for epoch in range(config.Epoch):
            for step in range(int(config.train_num / config.train_batch_size)):
                ...
                train_loss, train_accuracy, _, _ = sess.run([loss, accuracy, train_op, extra_update_ops], {images : images_train.eval(), bbox_true_13 : bbox_true_13_train.eval(), bbox_true_26 : bbox_true_26_train.eval(), bbox_true_52 : bbox_true_52_train.eval(), is_training : True})
                duration = time.time() - start_time
                examples_per_sec = float(duration) / config.train_batch_size
                format_str = ('Epoch {} step {},  train loss = {} train accuracy {} ( {} examples/sec; {} ''sec/batch)')
                print(format_str.format(epoch, step, train_loss, train_accuracy, examples_per_sec, duration))
            ...
            for step in range(int(config.val_num / config.val_batch_size)):
                start_time = time.time()
                val_loss, val_accuracy = sess.run([loss, accuracy], {images: images_val.eval(), bbox_true_13: bbox_true_13_val.eval(), bbox_true_26: bbox_true_26_val.eval(), bbox_true_52: bbox_true_52_val.eval(), is_training: False})
                duration = time.time() - start_time
                examples_per_sec = float(duration) / config.val_batch_size
                format_str = ('Epoch {} step {}, val loss = {} val accuracy {} ({} examples/sec; {} ''sec/batch)')
                print(format_str.format(epoch, step, val_loss, val_accuracy, examples_per_sec, duration))
        ...
        with tf.device(tf.train.replica_device_setter(worker_device = "/job:worker/task:%d" % task_index, cluster = cluster)):
            ...
            with tf.Session(config = tf.ConfigProto(log_device_placement = False)) as sess:
                ...
                for step in range(int(config.Epoch * config.train_num / config.train_batch_size)):
                    loss_value, accuracy_value, _, _, pred = sess.run(
                        [loss, accuracy, train_op, extra_update_ops, pred_object],
                    ...
                    if step % 2 == 0:
                        start_time = time.time()
                        train_loss, train_accuracy = sess.run([loss, accuracy], {images: images_train.eval(), bbox_true_13: bbox_true_13_train.eval(), bbox_true_26: bbox_true_26_train.eval(), bbox_true_52: bbox_true_52_train.eval(), is_training: True})
                        duration = time.time() - start_time
                        examples_per_sec = float(duration) / config.train_batch_size
                        format_str = ('step {},  train loss = {} train accuracy {} ( {} examples/sec; {} ''sec/batch)')
                        print(format_str.format(step, train_loss, train_accuracy, examples_per_sec, duration))
                    ...
                    if step > 1 and step % 20 == 0:
                        ...
                        duration = time.time() - start_time
                        examples_per_sec = float(duration) / config.val_batch_size
                        format_str = ('step {}, val loss = {} val accuracy {} ({} examples/sec; {} ''sec/batch)')
                        print(format_str.format(step, val_loss, val_accuracy, examples_per_sec, duration))
                        summary_writer.add_summary(summary = tf.Summary(value=[tf.Summary.Value(tag="val loss", simple_value = val_loss)]), global_step = step)
                        summary_writer.flush()
